...
L<http://stp.lingfil.uu.se/~joerg/download.php?f=Lingua-Align.tar.gz>
...
=head3 Run tests on existing data sets
...
The example training data is stored in C<europarl/nl-en-weak_125.xml> which has been produced by manual alignment (thanks to Gideon Kotze) using the Stockholm Tree Aligner L<http://kitt.cl.uzh.ch/kitt/treealigner>. The format looks like this:
...
=head3 Run with your own settings
...
Hopefully this will work to create a corpus that can be loaded into the Stockholm Tree Aligner. You might have to check the specifications in the XML header and adjust some (meta) information.

=head4 Features & parameters
...
This should give you very low precision and recall values (around 25%; Note that individual recall values for terminal nodes and non-terminal nodes are not correct because nod type information is not available in the gold standard and IDs do not follow the standard to make a clear distinction).
...
Don't ask me why the parameter is '-y' for the Moses algnment file. (It's basically because treealign only uses short command-line options and I was running out of letters ....)
...
Note that history features coming from parent links and coming from children cannot be combined (for obvious reasons). And don't expect improvements in all cases. Especially for rich feature sets no big improvements can be expected. Note that alginment will also be (even) slower.


=head4 Alignment strategies
...
=head3 Library structure

There are several options that can be set. For further information have alook at the manpages linked below or just look at the source code. Extending the code is quite straightforward even though the documentation is not perfect and the coee is partly awful (well, it's Perl ....). Here is a (hopefully up-to-date) list of Modules (many of them are under-developed / experimental / non-functioning possible projects for the future):
...
=head3 How to do word alignment
...
As features you may use, for example, string similarity measures such as LCSR score (longest common sub-sequence reatio), Dice scores based on co-occurrence frequencies, Moses/Giza++ alignments, binary features such as the occurrence of suffix pairs etc. Run C<make> in the C<europarl/wpt03> to see an example alignment experiment.
...
=head2 Documentation & References
...
Tiedemann, J. Lingua-Align: An Experimental Toolbox for Automatic Tree-to-Tree Alignment. In Proceedings of the 7th International Conference on Language Resources and Evaluation (LREC'2010), 2010.
L<http://stp.lingfil.uu.se/~joerg/published/lrec2010.pdf>,

Tiedemann, J. and Kotzé, G. Building a Large Machine-Aligned Parallel Treebank. In Proceedings of the 8th International Workshop on Treebanks and Linguistic Theories (TLT'08), pages 197-208, EDUCatt, Milano/Italy, 2009.
L<http://stp.lingfil.uu.se/~joerg/published/tlt09.pdf>,

Tiedemann, J. and Kotzé, G. A Discriminative Approach to Tree Alignment. In Proceedings of the International Workshop on Natural Language Processing Methods and Corpora in Translation, Lexicography and Language Learning (in connection with RANLP'09), pages 33 - 39, 2009.
