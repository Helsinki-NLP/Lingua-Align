...
use vars qw($opt_f $opt_t $opt_a $opt_s $opt_c $opt_m $opt_C $opt_S $opt_P $opt_D);
use Getopt::Std;

getopts('f:t:a:s:c:m:SCPD');
...
my $aligntrain = new Lingua::Align::Trees(
    ...
    -keep_training_data => 0,             # don't remove feature file
...
$aligntrain->train(\%corpus,'sophietrain.'.$model,$nrTrain);
...
my $algfile = $opt_c || 'Alignments_SMULTRON_Economy_Texts_SV_EN.xml';
...
#    -terminals_only => 1,                 # link terminals only
    ...
    -lexe2f => 'moses-eco/model/lex.0-0.e2f',
    -lexf2e => 'moses-eco/model/lex.0-0.f2e',
#    -lexe2f => 'moses-all/model/lex.0-0.e2f',
#    -lexf2e => 'moses-all/model/lex.0-0.f2e',


    ## for the GIZA++ word alignment features
    -gizaA3_e2f => 'moses-eco/giza.src-trg/src-trg.A3.final.gz',
    -gizaA3_f2e => 'moses-eco/giza.trg-src/trg-src.A3.final.gz',
#    -gizaA3_e2f => 'moses-all/giza.src-trg/src-trg.A3.final.gz',
#    -gizaA3_f2e => 'moses-all/giza.trg-src/trg-src.A3.final.gz',

    ## for the Moses word alignment features
    -moses_align => 'moses-eco/model/aligned.intersect',
#    -moses_align => 'moses-sophie/model/aligned.grow-diag-final-and',
#    -moses_align => 'moses-all/model/aligned.grow-diag-final-and',
...
$treealigner->align(\%corpus,'sophietrain.'.$model,$search,$nrAlign);
